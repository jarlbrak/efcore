...
/// </summary>
public sealed class StringDictionaryComparer<TDictionary, TElement> : ValueComparer<TDictionary>
    where TDictionary : IDictionary<string, TElement>
{
    /// <summary>
    ///     This is an internal API that supports the Entity Framework Core infrastructure and not subject to
    ///     the same compatibility standards as public APIs. It may be changed or removed without notice in
    ...
    private static bool Compare(TDictionary? a, TDictionary? b, ValueComparer elementComparer)
    {
        if (a is null)
        {
            return b is null;
    ...
    private static int GetHashCode(TDictionary source, ValueComparer elementComparer)
    {
    ...
    private static TDictionary Snapshot(TDictionary source, ValueComparer elementComparer)
    {
        var snapshot = (source is ICloneable cloneable)
            ? (TDictionary)cloneable.Clone()
            : (TDictionary)Activator.CreateInstance(source.GetType())!;
